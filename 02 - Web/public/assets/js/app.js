...
async function init() {
  try {
    database = await loadTroubleshootingDatabase();
...
async function loadTroubleshootingDatabase() {
  const tauriInvoke = window.__TAURI__?.core?.invoke;

  if (typeof tauriInvoke === "function") {
    return await tauriInvoke("load_troubleshooting_database");
  }
...
function showLoadError(error) {
  const message = error?.message || String(error || "Erro desconhecido ao carregar a base.");
  ...
  elements.detailContainer.innerHTML = `
    <div class="load-error">
      ...
      <p>${safe(message)}</p>
      <p>Verifique se a base de troubleshooting foi incluída no aplicativo.</p>
    </div>
  `;
}
